...
from sklearn.metrics import roc_curve, accuracy_score, precision_score, recall_score, f1_score, roc_auc_score, confusion_matrix, RocCurveDisplay, ConfusionMatrixDisplay
...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        ...
        for img, label in tqdm(test_loader, desc="Testing"):
            ...
            label = label.float().unsqueeze(1)
            orig_label = label.squeeze(1)
            
            prediction = model(img)
            probs = torch.sigmoid(prediction).squeeze(1)
            preds = probs.round()
         
            test_acc.append(preds.eq(orig_label).float().mean().item())

            all_preds.extend(preds.cpu().numpy())
            all_labels.extend(orig_label.cpu().numpy())
            all_probs.extend(probs.cpu().numpy())

        mean_test_acc = torch.as_tensor(test_acc).mean().item()
        print(f"Test accuracy: {mean_test_acc * 100:.2f}%")
